...
INSERT INTO bronze."lims_houston_samples" (source_row_number, "sample_id", "product_line", "material_code", "batch_lot_number", "container_id", "test_type", "test_method_version", "instrument_id", "analyst_id", "reviewer_id", "date_requested", "date_received", "date_started", "date_completed", "priority", "submitter", "project_reference", "result_value", "result_unit", "spec_lower_limit", "spec_upper_limit", "sample_status", "approval_status", "approval_date", "storage_location", "retest_flag", "comments", "site_code")
    ...
    ELSE 'DD/MM/YYYY' END) || CASE WHEN length(raw_data ->> 'DATE_REQUESTED') = 10 THEN '' WHEN length(raw_data ->> 'DATE_REQUESTED') = 16 THEN ' HH24:MI' ELSE ' HH24:MI:SS' END)
    ...
    ELSE (raw_data ->> 'DATE_COMPLETED')::timestamp END, raw_data ->> 'PRIORITY', raw_data ->> 'SUBMITTER', raw_data ->> 'PROJECT_REFERENCE', NULLIF(raw_data ->> 'RESULT_VALUE', '')::numeric, raw_data ->> 'UOM', NULLIF(raw_data ->> 'SPEC_LOWER_LIMIT', '')::numeric, NULLIF(raw_data ->> 'SPEC_UPPER_LIMIT', '')::numeric, raw_data ->> 'SAMPLE_STATUS', raw_data ->> 'APPROVAL_STATUS', CASE
    ...
    ELSE 'DD/MM/YYYY' END) || CASE WHEN length(raw_data ->> 'APPROVAL_DATE') = 10 THEN '' WHEN length(raw_data ->> 'APPROVAL_DATE') = 16 THEN ' HH24:MI' ELSE ' HH24:MI:SS' END)
    ELSE (raw_data ->> 'APPROVAL_DATE')::timestamp END, raw_data ->> 'STORAGE_LOCATION', CASE lower(raw_data ->> 'RERUN_FLAG') WHEN 'true' THEN true WHEN 'false' THEN false ELSE NULL END, raw_data ->> 'COMMENTS', raw_data ->> 'SITE_CODE'
