...
INSERT INTO bronze."tox_trial" (source_row_number, "trialid", "compoundid", "trialtype", "cro_name", "studyprotocolid", "trialdate", "speciestested", "exposureroute", "exposurelevel_mgkg", "exposureduration_days", "toxicityscore_1to5", "ld50_value", "noael_value", "outcomestatus", "regulatoryframework", "reportreference")
            SELECT row_number, raw_data ->> 'TRIAL_ID', raw_data ->> 'COMPOUND_ID', raw_data ->> 'TRIAL_TYPE', raw_data ->> 'CRO_NAME', raw_data ->> 'STUDY_PROTOCOL_ID', NULLIF(raw_data ->> 'TRIAL_DATE', '')::timestamp, raw_data ->> 'SPECIES_TESTED', raw_data ->> 'EXPOSURE_ROUTE', NULLIF(raw_data ->> 'EXPOSURE_LEVEL_MGKG', '')::numeric, NULLIF(raw_data ->> 'EXPOSURE_DURATION_DAYS', '')::numeric, NULLIF(raw_data ->> 'TOXICITY_SCORE_1TO5', '')::numeric, NULLIF(raw_data ->> 'LD50_VALUE', '')::numeric, NULLIF(raw_data ->> 'NOAEL_VALUE', '')::numeric, raw_data ->> 'OUTCOME_STATUS', raw_data ->> 'REGULATORY_FRAMEWORK', raw_data ->> 'REPORT_REFERENCE'
