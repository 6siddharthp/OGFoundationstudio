...
CREATE OR REPLACE PROCEDURE OGFS_DEMO.SILVER.run_downstream()
...
BEGIN
  ...
  SELECT 'downstream_dispatcher',LISTAGG(DISTINCT source_table,','),'SUCCEEDED',COUNT(*),CURRENT_TIMESTAMP(),CURRENT_TIMESTAMP()
  FROM OGFS_DEMO.BRONZE.orchestration_events_stream;
...
END;
$$;
CREATE OR REPLACE TASK OGFS_DEMO.SILVER.downstream_dispatcher_task
  WAREHOUSE = OGFS_DEMO_WH SCHEDULE = '5 MINUTE'
  USER_TASK_TIMEOUT_MS = 3600000
  SUSPEND_TASK_AFTER_NUM_FAILURES = 2
  WHEN SYSTEM$STREAM_HAS_DATA('OGFS_DEMO.BRONZE.orchestration_events_stream')
AS CALL OGFS_DEMO.SILVER.run_downstream();
